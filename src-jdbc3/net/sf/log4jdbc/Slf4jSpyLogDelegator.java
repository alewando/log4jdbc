...
public class Slf4jSpyLogDelegator implements SpyLogDelegator
{
   ...
   *
   * if the optional log4jdbc.debug.ignore.packages system property is defined then
   * the last call point from an application in a non-ignored package is shown in the debug
  ...
  private static String getDebugInfo()
  {
    ...
    if (stackTrace != null)
    {
      ...
      else
      {
        ...
        outer: for (int i = 0; i < stackTrace.length; i++)
        {
          className = stackTrace[i].getClassName();
          if (className.startsWith("net.sf.log4jdbc"))
          {
          ...
          else if (DriverSpy.TraceFromApplication && firstLog4jdbcCall != 0)
          {
            // Check if element is in ignore list
            for (int j = 0; j < DriverSpy.DebugIgnorePackages.length; j++) {
              if (className.startsWith(DriverSpy.DebugIgnorePackages[j])) {
                continue outer;
              }
            }        	  
